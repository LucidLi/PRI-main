...
def single_resp(idx, prompt, judge_model, api):
    model = model_utils.GPT(judge_model, api=api)     # gpt-4-0613
    response = model(prompt)
    return idx, response

class GPTJudge:
    def __init__(self, policy, mp=1, judge_model='gpt-4o', api=None):  # gpt-4-0613
    ...
    def evaluate(self, QApairs):
        ...
        responses = []
        model = model_utils.GPT(self.judger, api=self.api)   # gpt-4-0613
                ...
                responses = tqdm(
                    list(
                        pool.starmap(
                            single_resp,
                            [
                                (idx, judge_template % QApair, self.judger, self.api)
